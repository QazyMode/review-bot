...
describe('Core', () => {
  it('Должен уведомить двух ревьюеров при открытии МР', async () => {
    ...
    expect(notifier.notifyReviewerAboutMr).toHaveBeenCalledWith('someuser', {
      author: 'aleksey.mastekha',
      link: 'https://github.com/QazyMode/review-bot/rb188468/frontend-rmb-test/-/merge_requests/5',
  ...
  it('должен уведомить автора МР о том, что ревьюер поставил апрув', async () => {
    ...
    expect(notifier.notifyAuthorAboutApprove).toHaveBeenCalledWith('author', {
      link: 'https://github.com/QazyMode/review-bot/rb188468/frontend-rmb-test/-/merge_requests/8',
...
function prepare() {
  ...
  const core = new Core(userCommandsHandler);
